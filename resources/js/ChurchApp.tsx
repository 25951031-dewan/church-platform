...
export default function App() {
    ...
    // Admin panel takes over the full viewport — no top nav
    ...
    const renderPage = () => {
        if (pathname.startsWith('/events'))    return <EventsPage />;
        if (pathname.startsWith('/community')) return <CommunityPage />;
        if (pathname.startsWith('/faq'))       return <FaqPage />;
        if (pathname.startsWith('/profile'))   return <ProfilePage userId={user.id} />;
        return <FeedPage />;
    };
        ...
        <div className="min-h-screen bg-gray-50">
            <nav className="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-10">
                <div className="max-w-4xl mx-auto px-4 flex items-center justify-between h-14">
                    <button onClick={() => navigate('/')} className="font-bold text-indigo-600 text-lg">
                        Church Platform
                    </button>
